...
#计算时间间隔
def getInterval(date):
    # start=datetime.datetime.strptime(str(date),"%Y-%m-%d %H:%M:%S")
    # end=datetime.datetime.strptime(str(datetime.datetime.now()),"%Y-%m-%d %H:%M:%S")
    Interval=datetime.datetime.now()-date
    if Interval.days!=0:
        return str(Interval.days)+'天前'
    if int(Interval.seconds/3600)!=0:
        return str(int(Interval.seconds/3600))+'小时前'
    if int(Interval.seconds/60)!=0:
        return str(int(Interval.seconds/60))+'分钟前'
    return str(Interval.seconds)+'秒前'
'''
根据某个字段获取一个dic出来
'''
